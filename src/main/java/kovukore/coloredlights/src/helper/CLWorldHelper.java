...
//            }	
	...
	//Copied from the world class in 1.7.2, modified from the source from 1.6.4, made the method STATIC
	//Added the parameter 'World world, ' and then replaces all instances of world, with WORLD
	public void updateLightByType(World world, EnumSkyBlock par1Enu, int x, int y, int z)
    {
    	if (world.doChunksNearChunkExist(x, y, z, 17))
        {
            ...
            int compLightValue = computeLightValue(world, x, y, z, par1Enu);
            long l1;
            int x1;
            int y1;
            int z1;
            ...
            {
                lightUpdateBlockList[i1++] = 133152;		//Save Entry at pos 0 (move i1)
            }
            else if (compLightValue < savedLightValue)
            {
                lightUpdateBlockList[i1++] = 133152 | savedLightValue << 18;	//Save Entry at pos 0 with its Light Value (move i1)	

                while (l < i1)
                {	  
                    l1 = lightUpdateBlockList[l++];		//Get Entry at l, which starts at 0
                        ...
                        if (lightEntry > 0)
                        {
                            ...
                            if (x2 + y2 + z2 < 17)
                            {
                            	for (int faceIndex = 0; faceIndex < 6; ++faceIndex)
                                {
                                     ...
                                     Block block = world.getBlock(xFace, yFace, zFace);
                                     int blockOpacity = (block == null ? 0 : block.getLightOpacity(world, xFace, yFace, zFace));
                                     ...
                                     if ( ((expectedEntryLight&15) >= ll) && (i1 < lightUpdateBlockList.length))
                                    	 lightUpdateBlockList[i1++] = xFace - x + 32 | (yFace - y + 32 << 6) | (zFace - z + 32 << 12) | ((ll | rl | gl | bl) << 18);
                                }
                            }
                        }
                    }
            ...
            while (l < i1)
            {
                l1 = lightUpdateBlockList[l++];		//Get Entry and it's light value (if there is one)
                x1 = ((int)(l1 & 63) - 32 + x);				//Get Entry X coord
                y1 = ((int)(l1 >> 6 & 63) - 32 + y);		//Get Entry Y coord
                z1 = ((int)(l1 >> 12 & 63) - 32 + z);		//Get Entry Z coord
                ...
                //Compute the light level at the entry's location.  If the light's have been set to zero before world occurs,
                //then the computation will change dynamically
                expectedEntryLight = computeLightValue(world, x1, y1, z1, par1Enu);
